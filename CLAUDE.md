...
- **The window is one `QGridLayout` (`main_grid`) so the two columns' top rows line up.** Qt has no cross-layout notion of a row, so while the columns were two independent `QVBoxLayout`s their rows only ever aligned by accident — the separators sat 7px apart and the rows above them 6px and 10px, since the left rows hold a label and a button (h≈19) against the right's `QPlainTextEdit` fields (h=25). The first two rows and the separator beneath them are now shared grid rows: column 0 is the left panel, column 1 the divider (`line_5`, `rowspan=4`, which is what keeps the vertical rule unbroken), column 2 the right-hand rows, column 3 the logo (`rowspan=2`). Row 3 holds each column's remaining items in the `QVBoxLayout`s that used to be the columns, and takes the vertical slack (`rowstretch="0,0,0,1"`, `columnstretch="0,0,1,0"`). Two consequences to know before editing this in Designer: the outer margins had to move to the grid, because those `QVBoxLayout`s now cover only row 3 and their own margins would otherwise indent it away from the rows above; and the fixed 8px `spacer_above_line_7` is gone, the grid row positioning the separator instead. `tests/test_commander_ui_loads.py` asserts the paired rows share a top and a bottom exactly — which a grid guarantees and no font size affects — so a Designer edit that breaks the shared rows fails rather than merely looking wrong.
