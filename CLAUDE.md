...
- **One method owns the file dialogs' pane widths.** `_set_dialog_pane_widths()` sets the sidebar's width and the preview's in a single `setSizes`, and all three dialogs call it once the pane (if any) is in place. Single deliberately, and stating every pane's width except the listing's, because it runs on a dialog that has not been shown: the splitter reports placeholder sizes rather than the widths Qt restored, so a second call throws the first call's widths away, and a pane left out of the list is squashed to its minimum. That second trap was live for a while — every dialog carrying a preview showed a 90px `Co...` sidebar however wide Qt had it, and wrote that width back as the one to remember. The listing is the one pane left to the splitter, being the one that should take what is left over. Those placeholders are also useless for measuring how much wider the dialog should be, so each pane says that for itself: the preview counts its whole width, being new, and the sidebar only what it gains over `sizeHint()`. `_sidebar_widening()` is a first-run default and only that — Qt persists `FileDialog/sidebarWidth` in shared user settings (`QT_SETTINGS_ORGANISATION`, read by `_remembered_sidebar_width()`) and restores it when a dialog is built, so a remembered width is the width that gets set; overriding it every time would mean a narrower sidebar could never be kept, and Commander writing its own width back over the user's choice. A setting that will not make an integer — renamed on some future Qt — reads as 'nothing remembered', which is the harmless way round. The dialog is widened only up to `SIDEBAR_PANE_WIDTH`, since a sidebar dragged wider than that is a listing the user has chosen to shrink. In the tests, `qt_sidebar_width` says what is remembered and `_preserve_qt_sidebar_width` puts the machine's own value back from `atexit` — the `QApplication` outlives every fixture, so a teardown-time restore lost the race, and a run that left 90 behind turned the widening off on the developer's own machine.
