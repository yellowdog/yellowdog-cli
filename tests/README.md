...
| `test_commander_help.py` | The Help dialog: the README declared as package data, every heading given its GitHub anchor so that every section link in the README lands, code blocks shaded and the screenshot dropped, the Help button and F1 opening one modeless dialog that keeps the reader's place, Back returning to where a link was followed from, and a missing README reported in the dialog |
