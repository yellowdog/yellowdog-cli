...
| `test_variable_names.py` | The variable naming rule (`VARIABLE_NAME_PATTERN`): an error at every definition source (`-v`, `YD_VAR_*`, `[common.variables]`, `--property common.variables.<name>`), leading digits allowed, a name breaking it treated as text rather than a reference, and `env:` names allowed what the OS allows |
...
| `test_variable_subs.py` | `utils/variables.py` — `{{variable}}` substitution engine, including the rendering of non-string variable values as JSON so that they round-trip through the type tags, that a string is never requoted, that a TOML date falls back to its text, that nested variables resolve alike in JSON, Jsonnet and TOML specifications, that a compact (unindented) specification keeps its type-tagged values' types, that an unset (`::`) expression nested in another removes the property when its value is needed, that substitution passes — in situ, over file contents and for single values — repeat until nothing changes, reporting a circular reference as an error, and that an undefined variable is warned of once, from the point the command wrappers enable it, and never for lazy variables, non-variable text or Mustache, including by the check-only `warn_of_undefined_variables()` |
| `test_variables_command.py` | `variables.py` — `yd-variables`: named selection, all variables when none is named, alphabetical order, `null` for a name that isn't a variable, and the redaction of `key`/`secret` in the full report (revealed by naming them or by `--show-secrets`, and no other variable redacted), and the output's shape, as `yd-show`'s: the JSON followed by `Done`, and the JSON alone under `--quiet`; the undefined-variable warnings: printed ahead of the JSON (the `[common]` ones included), covering only the variables reported, never for the credentials, and suppressed by `--quiet`, which leaves parseable JSON — Commander's invocation included; and the explanation of a variable the `::` syntax removed (`explain_unset_variable()`): bare and embedded `{{::}}`, an undefined `{{x::}}`, type tags, `env:`, references inside nested expressions, the `{{x::}}` chain followed to its end, and the definitions record kept in step with redefinition; and that unset is not transitive by plain reference (left unsubstituted and warned of as *unset*, whatever the definition order) while a `{{x::}}` reference still propagates it |
...
| `test_commander_selection_labels.py` | Selected definition files shown on their own 'Select' buttons, with the full path as the tooltip and the original tooltip restored on deselection, without widening the left-hand column |
...
| `test_commander_help.py` | The Help dialog: the README declared as package data, every heading given its GitHub anchor so that every section link in the README lands, code blocks shaded and the screenshot dropped, the README's table of contents present and drawn tight while prose lists keep their spacing, the Help button and F1 opening one modeless dialog that keeps the reader's place, Back returning to where a link was followed from, and a missing README reported in the dialog |
...
| `test_commander_output_filter.py` | Filtering the output window to one command's run, with real child processes: every line of the run kept (continuation lines, tables and the `Executing:` line) and no other, the `Executing:` line offering both its command and Commander and belonging to both views, the right-click menu and its wiring, the filter bar and its hidden-line count, Esc and Show All, Clear ending the filter, Copy and Save taking what is shown, the right-clicked or top line kept in place and a followed end kept followed, and the block count the attribution relies on agreeing with Qt's. Also the process chooser: which runs it lists (a cleared, finished one not, a running one yes) and in what order, the command line as the user gave it, the outcome and line-count columns lining up, the rows ticked when it opens, one or several runs shown (with Commander, and with output arriving for any of them) and the bar naming each, Show Output greyed with nothing ticked, accepting by button or Return, Cancel changing nothing, and its width fitting its rows up to the window's |
| `test_commander_logging.py` | How a command is echoed into the output window; many YDIDs collapse to a count |
| `test_commander_config_discovery.py` | The `yd-variables` run behind the placeholders: what each failure reports, that a timed-out discovery is retried once with a longer budget, that every path into discovery gets that retry, that the debounced reparse waits for a half-typed user variable to be completed, and that the one failure meaning 'nothing is configured yet' is suppressed while no configuration file is selected without suppressing any other; and that the real invocation passes `--quiet`, since `yd-variables` prints its warnings ahead of the JSON |
...
| `test_commander_ui_loads.py` | `commander.ui` loads against the installed PyQt6, every code-referenced widget exists, and the layout holds together: the separator below the Browse Config Directory row lies between that row and the one beneath it, the paired top rows of the two columns share a grid row, the window opens tall and wide enough for its own layout, with no button squeezed below or stretched beyond its natural height, a taller window does not spread the left column's rows apart, each column's rows start at one left edge, no label is indented with leading spaces, a style change re-aligns the checkbox indents (the indents themselves need a real platform to appear), and every button, checkbox and field but the output window has a tooltip of at most two short lines |
...
| `test_commander_launcher.py` | `yd-commander`'s own options as `launcher.parse_args()` turns them into `StartupSettings`: every option, relative definition paths resolved against the launch directory, and what is refused — a missing config or definition file, the config file given twice, a variable not of the form `name=value` or containing whitespace, and a field value that is empty or holds a tab or newline (pure Python, no Qt) |
| `test_commander_startup_fields.py` | Applying those options to the window: the fields filled and still editable, the definition files selected on their buttons and reaching `yd-submit`, and the first config discovery being the only one and already seeing them |
...
| `commander_dialogs.py` | Drivers for Commander's own confirmation, chooser, process chooser and notice, for the cases where production builds and execs the dialog; `NOTHING` leaves a dialog to close itself, for a key it acts on after a delay |
