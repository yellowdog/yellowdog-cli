...
| `conftest.py` | `qapp` (one offscreen `QApplication`), `_gui_harness_guard` (surfaces what happened inside Qt callbacks), `_no_config_discovery` (stubs `_parse_yd_config`, so no test spawns `yd-show`; opt out with `@pytest.mark.real_config_parse`), and `qt_sidebar_width` with `_preserve_qt_sidebar_width` (set what Qt remembers for the file dialog's sidebar width, and put the machine's own value back afterwards — Qt keeps it in shared user settings, not in anything the session owns) |
