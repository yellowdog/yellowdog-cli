...
To pick from every process instead, right-click anywhere in the output window and choose **Show Output from Process…**. The dialog lists Commander's own messages and then each command in the order it started, with its PID, its command line as you gave it (without the configuration and formatting options Commander adds), when it started, whether it is still running or how it ended, and how many lines of output it has in the window. A command whose output has been cleared is not listed unless it is still running. The row selected when the dialog opens is the filter being shown, otherwise the command of the line you right-clicked, otherwise the latest. Choose one and click **Show Output**, press Return, or double-click it; hover a row for its full command line.
